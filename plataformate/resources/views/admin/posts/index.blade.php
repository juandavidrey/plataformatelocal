    ...
    <table id="posta-table" class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $post->resumen }}</td>
          <td>
            <a href="{!! url('grupos', $post->url) !!}" class="btn btn-xs btn-default" target="_blank"><i class="fa fa-eye"></i>
                  </a>
            ...
            <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" style="display:inline;">
              ...
              <button class="btn btn-xs btn-danger" onclick="return confirm('¿Estás seguro de querer eliminar este grupo?')"><i class="fa fa-times"></i></button>
            </form>
          </td>
        </tr>
        @endforeach
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group {{ $errors->has('ngrupo') ? 'has-error': '' }}">
            ...
            <input name="ngrupo" type="text" class="form-control" value="{{ old('ngrupo') }}" placeholder="Ingresa el nombre del grupo"> {!! $errors->first('ngrupo', '<span class="help-block">:message</span>') !!}
