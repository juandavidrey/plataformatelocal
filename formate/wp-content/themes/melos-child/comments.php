...
?>
	...
	<?php if ( have_comments() ) : ?>
		...
		<div id="comments-title">
			<h3>
				<?php
					printf( _n( '<span>%1$s</span> Comments', '<span>%1$s</span> Comments', get_comments_number(), 'melos' ),
	...
	<?php 
		...
		$comments_args = array(
			...
			'comment_field' =>  
				...
				'<textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Your Message', 'melos' ) . '" cols="45" rows="8" aria-required="true">' .
				'</textarea></p>',
			'fields' => apply_filters( 'comment_form_default_fields', array (
				'author' =>
					'<p class="comment-form-author one_third">' .
					'<input id="author" name="author" placeholder="' . esc_attr__( 'Your Name (Required)', 'melos' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
					'" size="30"' . $aria_req . ' /></p>',
				'email' =>
					'<p class="comment-form-email one_third">' .
					'<input id="email" name="email" placeholder="' . esc_attr__( 'Your Email (Required)', 'melos' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
					'" size="30"' . $aria_req . ' /></p>',
				'url' =>
					'<p class="comment-form-url one_third last">' .
					'<input id="url" name="url" placeholder="' . esc_attr__( 'Your Website', 'melos' ). '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) .
					'" size="30" /></p>'
			) ),
		);
