		...
		echo '<li>',
			...
			if ( ! empty( $slide['slide_url'] ) ) {
				...
				if ( empty( $slide['slide_button'] ) ) {
					$slide['slide_button'] = __( 'Ver más', 'melos' );
				}
...
$slider_toggle           = NULL;
	...
	if ( is_front_page() or thinkup_check_ishome() or $_SERVER['REQUEST_URI'] == '/formate/' ) {
...
global $thinkup_homepage_section3_button;
	...
	if ( empty( $thinkup_homepage_section1_title ) ) $thinkup_homepage_section1_title = 'Perfecto para todo';
	if ( empty( $thinkup_homepage_section2_title ) ) $thinkup_homepage_section2_title = '100% Adaptable';
	if ( empty( $thinkup_homepage_section3_title ) ) $thinkup_homepage_section3_title = 'Poderoso Framework';

	// Set default values for descriptions
	if ( empty( $thinkup_homepage_section1_desc ) ) 
	$thinkup_homepage_section1_desc = 'Melos se convierte en la opción perfecta para cualquier sitio web. Blog, negocios, caridad, ¡todo! ¡Ahora puede hacerlo a través de la belleza de Melos!';

	if ( empty( $thinkup_homepage_section2_desc ) ) 
	$thinkup_homepage_section2_desc = 'Melos es 100% adaptable. ¡Se ve bien en todos los dispositivos, desde teléfonos móviles hasta computadoras de escritorio!. Active el diseño adaptativo desde el panel de opciones del tema.';

	if ( empty( $thinkup_homepage_section3_desc ) ) 
	$thinkup_homepage_section3_desc = 'Pruebe nuestro increíble Framework de ThinkUpThemes. ¡Haga cambios sin tocar el código! ¡Tome el control y construya rápidamente su sitio web!';
	...
	if ( empty( $thinkup_homepage_section1_button ) )
		$thinkup_homepage_section1_button = __( 'Ver más', 'melos' );
	if ( empty( $thinkup_homepage_section2_button ) )
		$thinkup_homepage_section2_button = __( 'Ver más', 'melos' );
	if ( empty( $thinkup_homepage_section3_button ) )
		$thinkup_homepage_section3_button = __( 'Ver más', 'melos' );
