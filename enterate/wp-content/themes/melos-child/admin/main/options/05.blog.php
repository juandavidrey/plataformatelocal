...
function thinkup_input_blogtitle() {
	...
	echo	'<h2 class="blog-title">',
			'<a href="' . get_permalink() . '" title="' . esc_attr( sprintf( __( 'Enlace permanente a %s', 'melos' ), the_title_attribute( 'echo=0' ) ) ) . '">' . get_the_title() . '</a>',
			'</h2>';
}
...
function thinkup_input_blogauthor() {
	printf( __( '<span class="author"><a href="%1$s" title="%2$s" rel="author">%3$s</a></span>', 'melos' ),
		...
		esc_attr( sprintf( __( 'Ver todas las publicaciones de %s', 'melos' ), get_the_author() ) ),
		get_the_author()
	);
}
...
$class_button = NULL;
	...
	return '<p class="more-link"><a href="'. get_permalink($post->ID) . '" class="' . $class_button . '">' . __( 'Leer más', 'melos') . '</a></p>';
}
...
global $thinkup_post_shareemail;
	...
	echo	'<div id="sharepost">';
		...
		if ( ! empty( $thinkup_post_sharemessage ) ) { 
			...
			echo '<p>Comparte esta publicación</p>'; 
		}
		echo	'</div>';
		...
		if ( $thinkup_post_sharetwitter == '1' ) {
			echo '<a class="shareicon twitter" onclick="MyWindow=window.open(&#39;//twitter.com/home?status=Mira%20esto%20' . $page_title . '%20at%20' . get_permalink() . '&#39;,&#39;MyWindow&#39;,width=650,height=450); return false;" href="//twitter.com/home?status=Check%20this%20out!%20' . $page_title . '%20at%20' . get_permalink() . '" data-tip="top" data-original-title="Twitter"><i class="fa fa-twitter"></i></a>';
		}
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			...
			<header>
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Tu comentario está a la espera de aprobación por parte de un moderador.', 'melos'); ?></em>
					<br />
				<?php endif; ?>
