			...
			<?php while ( have_posts() ) : the_post(); ?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<header class="entry-header">
						<div class="entry-meta">
							<?php
								...
								printf( __( 'Published ', 'melos') . '<span><time datetime="%1$s">%2$s</time></span> at <a href="%3$s" title="' . __( 'Link to full-size image', 'melos') . '">%4$s &times; %5$s</a> in <a href="%6$s" title="' . __( 'Return to ', 'melos') . '%7$s" rel="gallery">%8$s</a>',
					...
					<div class="entry-content">
						...
						<?php the_content(); ?>
						<?php wp_link_pages( array( 'anterior' => '<div class="page-links">' . __( 'Pages', 'melos') . ':', 'posterior' => '</div>' ) ); ?>
