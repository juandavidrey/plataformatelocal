<?php
	...
	$TotalSoftNew_Plugin = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_namenp WHERE New_Plugin_Name = %s AND Our_Plugin_Name = %s", 'Pricing Table', 'Poll'));
	if(count($TotalSoftNew_Plugin)==0)
	{
		$wpdb->query($wpdb->prepare("INSERT INTO $table_namenp (id, New_Plugin_Name, Our_Plugin_Name, Dismiss) VALUES (%d, %s, %s, %s)", '', 'Pricing Table', 'Poll', '0'));
	}
	$TotalSoftPoll_Quest = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_namenp WHERE New_Plugin_Name = %s AND Our_Plugin_Name = %s", 'Poll Question', 'Poll'));
	if(count($TotalSoftPoll_Quest)==0)
	{
		$wpdb->query($wpdb->prepare("INSERT INTO $table_namenp (id, New_Plugin_Name, Our_Plugin_Name, Dismiss) VALUES (%d, %s, %s, %s)", '', 'Poll Question', 'Poll', '0'));
	}
...
<style type="text/css">
	...
	.TS_Poll_Question_MDiv { position: relative; margin: 15px 0; width: 99%; padding: 10px; background: white; box-sizing: border-box; -moz-box-sizing: border-box; -webkit-box-sizing: border-box; }
	.TS_Poll_Question_Icon { font-size: 50px; color: #009491; }
	.TS_Poll_Question_H1 { color: #009491 !important; font-family: Andalus; margin: 0 0 10px; font-size: 24px !important; }
	.TS_Poll_Question_DisMiss { display: inline-block; margin: 0 10px; padding: 5px 10px; border: 1px solid #009491; width: 150px; border-radius: 24px; color: #009491; background: #ffffff; cursor: pointer; transition: all 0.5s; -moz-transition: all 0.5s; -webkit-transition: all 0.5s; }
	.TS_Poll_Question_DisMiss:hover { background: #009491; color: #ffffff; }
	.TS_Poll_Question_DisMiss:focus { outline: none !important; }
	.TS_Poll_Question_Ask { display: inline-block; margin: 0 10px; padding: 5px 10px; border: 1px solid #009491; width: 250px; border-radius: 24px; color: #ffffff; background: #009491; cursor: pointer; transition: all 0.5s; -moz-transition: all 0.5s; -webkit-transition: all 0.5s; text-align: center; text-decoration: none; }
	.TS_Poll_Question_Ask:hover { text-decoration: none; background-color: #ffffff; color: #009491; outline: none !important; box-shadow: none !important; -moz-box-shadow: none !important; -webkit-box-shadow: none !important; }
	.TS_Poll_Question_Ask:focus { text-decoration: none; outline: none !important; box-shadow: none !important; -moz-box-shadow: none !important; -webkit-box-shadow: none !important; }
</style>
<script type="text/javascript">
	function TS_PTable_New_MTable_Span(val)
	{
		...
		if(val == 'Dismiss')
		{
			...
			var data = {
			action: 'TS_PTable_New_MTable_DisMiss_Poll', // wp_ajax_my_action / wp_ajax_nopriv_my_action in ajax.php. Can be named anything.
			foobar: val, // translates into $_POST['foobar'] in PHP
			};
			jQuery.post(ajaxurl, data, function(response) {})
		}
	}
	function TS_Poll_Question_Button(val)
	{
		jQuery('.TS_Poll_Question_MDiv').animate({'height':'0px','opacity':'0','margin':'0 0','padding':'0'},500);

		if(val == 'Dismiss')
		{
			var ajaxurl = object.ajaxurl;
			var data = {
			action: 'TS_Poll_Question_DisMiss', // wp_ajax_my_action / wp_ajax_nopriv_my_action in ajax.php. Can be named anything.
			foobar: val, // translates into $_POST['foobar'] in PHP
			};
			jQuery.post(ajaxurl, data, function(response) {})
		}
...
<form method="POST">
	...
	<?php }?>
	<?php if($TotalSoftPoll_Quest[0]->Dismiss == '0'){ ?>
		<div class="TS_Poll_Question_MDiv">
			<table class="TS_PTable_New_MTable">
				<tr>
					<td style="width: 50px !important; position: relative;">
						<i class="totalsoft totalsoft-question-circle-o TS_Poll_Question_Icon"></i>
					</td>
					<td style="width: calc(100% - 50px) !important; position: relative; position: relative; font-size: 16px; font-family: Andalus;">
						<h1 class="TS_Poll_Question_H1">Total Soft Support Team</h1>
						Hello. <br>
						...
						<p style="position: relative !important;">
							<a href="https://wordpress.org/support/plugin/poll-wp" class="TS_Poll_Question_Ask" target="_blank">
								Ask Question
							</a>
							<input type="button" class="TS_Poll_Question_DisMiss" value="Dismiss" onclick="TS_Poll_Question_Button('Dismiss')">
							<input type="button" class="TS_Poll_Question_DisMiss" value="Close" onclick="TS_Poll_Question_Button('Close')">
						</p>
					</td>
				</tr>
			</table>
